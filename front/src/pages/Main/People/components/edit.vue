				...
				<div class="edit-main" v-show="user">
					<div v-show="user">
						<div class="userAvatarEditor" @click="selectFile">
							<div class="userAvatar" v-if="user.avatar"><img :src="imageServer+user.avatar" id="avatar-img" width="160" height="160"></div>
							...
							<input type="file" name="avatar" id="avatar-upload" ref="avatarUpload" accept="image/png,image/jpeg" style="display: none;">
						</div>
					</div>
...
import { userSaveInfoApi,getNowUserApi } from 'api/user'
import { setUserAvatarApi } from 'api/upload'
 	export default {
		...
		methods: {
			...
			ajaxUpload() {
				var file = this.$refs.avatarUpload.files[0]
 				var formdata = new FormData()
 				formdata.append('avatarFile',file)
        		formdata.append("userId", this.user._id)
        		setUserAvatarApi(formdata, (res)=>{
        			let avatarPath = global.imgServer + res.data.result.key
        			this.$router.push({name: 'cut',params: {path: avatarPath,user_url: this.user._id}})
        		})
				// var me = this;
				// $.ajaxFileUpload({  
				//     type: "POST",  
			...
			cutOver() {
				communicationMixin.$on('cutOver',(avatarHTTPURL)=> {
					this.user.avatar = avatarHTTPURL
					this.setUserAttributes([{key: 'avatar',value: avatarHTTPURL}])
				})
			},
			...mapMutations({
