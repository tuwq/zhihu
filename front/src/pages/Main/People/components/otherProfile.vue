<template>
	<div id="otherProfile">
		<div class="Card" v-show="people_detail_user">
			<div class="top"></div>
			<div class="bottom">
				<div class="userInfo-box">
					<div class="avatar-box">
						<div class="avatar-inner">
							<img :src="imageServer+people_detail_user.avatar" width="160" height="160" class="avatar-img">
						</div>
					</div>
					<div class="info" v-if="people_detail_user.info">
						<div class="info-header">
							<h1>
								...
								<span class="signature">{{people_detail_user.info.intro}}</span>
							</h1>
						</div>
						<div class="info-body">
							<span class="intro-font">个人简介</span>
							<span class="intro">{{people_detail_user.info.introduction}}</span>
						</div>
						...
						<div class="buttons">
							<button class="attention" v-show="people_detail_user.followStatus==0" 
							@click.stop.prevent="followPeople(people_detail_user._id,1)"><span>&#8203;<svg fill="currentColor" viewBox="0 0 24 24" width="1.2em" height="1.2em" style="margin-right: 8px;position: relative;top: 3px;"><path d="M13.491 10.488s-.012-5.387 0-5.998c-.037-1.987-3.035-1.987-2.997 0-.038 1.912 0 5.998 0 5.998H4.499c-1.999.01-1.999 3.009 0 3.009s5.995-.01 5.995-.01v5.999c0 2.019 3.006 2.019 2.997 0-.01-2.019 0-5.998 0-5.998s3.996.009 6.004.009c2.008 0 2.008-3-.01-3.009h-5.994z"></path></svg></span>关注{{people_detail_user.info.gender==0?'他':people_detail_user.info.gender==1?'他':'她'}}</button>
							<button class="attention cancel" v-show="people_detail_user.followStatus==1" type="button" 
							@click.stop.prevent="followPeople(people_detail_user._id,0)" 
	 						@mouseenter="enter($event)" 
	 						@mouseleave="leave($event)" style="color: #fff;background-color: #8590a6;">已关注</button>
 		...
 		methods: {
 			...
 			followPeople(target_id,action) {
 				followTargetApi({
 					...
 					this.setPeopleDetailUserFollowStatus(action)
 				})
 			},
 			...mapMutations({
				setPeopleDetailUserFollowStatus: 'SET_PEOPLE_DETAIL_USER_FOLLOWSTATUS'
			})
 		},
 		computed: {
			...mapGetters([
