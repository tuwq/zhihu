							...
							<div class="content">
								<div class="list-item" v-for="(item,index) in users">
									<div class="contentItem-image">
										<div class="user-link">
											<img :src="imageServer+item.avatar">
										</div>
									</div>
...
import { attentionQuestionReadApi } from 'api/attention'
	export default {
		...
		methods: {
			...
			followPeople(target_id,action,index) {
				followTargetApi({
 					...
 					action==1?this.users[index].fansSum++:this.users[index].fansSum--
				})
			},
			...mapMutations({
