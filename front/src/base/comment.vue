			...
			<div>
				<div class="item-meta">
					...
					@click.stop.prevent="toUser(item.user_id)"><div class="Popover">
						<div class="" id=""><a  target="_blank"><img class="user-link-avatar" :src="imageServer+item.user_id.avatar" width="24" height="24"></a></div>
					</div></span>
				...
				<div class="comment-editor" style="display: none;" ref="commentEditor">
...
import { commentVoteApi, commentInsertQuestionApi, commentInsertApi } from 'api/comment'
	export default {
		...
		methods: {
			openReply(e) {
				$(this.$refs.commentEditor).toggle()
			},
			closeReply(e) {
				$(this.$refs.commentEditor).hide()
			},
